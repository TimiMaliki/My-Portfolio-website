...
const Card = ({ icon, header, text , more , readmore}) => {
  ...
  return (
    <div className="w-full md:w-[300px] rounded-xl border shadow-[1.95px_1.95px_2.6px_rgba(0,0,0,0.15)] ovo-regular">
      ...
      <div className="text p-5"><p className="text-md md:text-lg font-normal flex md:flex justify-center md:justify-center gap-3">{text}</p></div>

      <div className="read-more w-full flex text-center align-middle justify-center ">
        <button onClick={()=> setReadMore(!readMore)} className="inline-flex text-md md:text-lg ovo-regular items-center justify-center relative outline-none cursor-pointer select-none touch-manipulation font-bold  h-10 px-4 py-2.5 rounded-lg  transition-colors duration-100">
          {readmore}
        </button>
      </div>
      {readMore && (
